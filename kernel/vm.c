...
kvminithart()
{
  w_satp(MAKE_SATP(kernel_pagetable)); // set the satp register->kernel_pagetable
...
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)  // 找到最终页表项addr
{
  if(va >= MAXVA)
    panic("walk");
...
kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  ...
  for(i = PGROUNDUP(start); i < start + sz; i += PGSIZE){
    ...
    flags = PTE_FLAGS(*pte) & ~PTE_U; //PTE_U flag set to 0 so that kernel can access it
